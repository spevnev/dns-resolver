...
int main() {
    ...
    check_response(unsigned_resolver.resolve("sub.name-error." UNSIGNED_DOMAIN, RRType::A));
    check_response(signed_resolver.resolve("sub.name-error." SIGNED_DOMAIN, RRType::A));
    check_response(nsec3_signed_resolver.resolve("sub.name-error." NSEC3_SIGNED_DOMAIN, RRType::A));
    return EXIT_SUCCESS;
}
